...
class FCCDN(nn.Layer):
    ...
    def __init__(self, in_channels, num_classes, os=16, use_se=True):
        ...
        self.conv_out_class = nn.Sequential(
            nn.UpsamplingBilinear2D(scale_factor=2),
            nn.Conv2D(
    ...
    def loss(logits, labels):
        label = paddle.argmax(labels, 1)
        # loss_aux =  fccdn_ssl_loss(logits[1], label)
        # label = paddle.squeeze(label, axis=1)
        # print(logits[0].shape, label.shape)
        loss_m = nn.CrossEntropyLoss(axis=1)(logits, label)
        return loss_m #+ loss_aux
