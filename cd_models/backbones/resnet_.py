...
# __all__ = ['ResNet', 'Res5Head', 'Blocks', 'BasicBlock', 'BottleNeck', 'ResbackBone']
...
class ResNet(nn.Layer):
    ...
    def __init__(self,
        ...
        if variant in ['c', 'd']:
            ...
            conv_def = [[in_channels, ch_in, 7, 2, conv1_name]]
...
class ResbackBone(nn.Layer):
    __shared__ = ['norm_type']

    def __init__(self,
                 depth=50,
                 ch_in=64,
                 lr_mult_list=[1.0, 1.0, 1.0],
                 groups=1,
                 base_width=64,
                 norm_type='bn',
                 norm_decay=0,
        ...
        block_nums = Resbb_cfg[depth]
        na = NameAdapter(self)

        
        self.ch_in = ch_in
        ch_out_list = [128, 256, 512]
        block = BottleNeck if depth >= 50 else BasicBlock
    ...
    def forward(self, x):
        outs = []
        for _, stage in enumerate(self.res_layers):
            x = stage(x)
            outs.append(x)
        return outs
...
if __name__ == "__main__":
    ...
    m = ResNet(in_channels=6, depth=34).to('gpu')
    # m = ResNet34_vd(in_channels=6).to('gpu')
    y = m(x)
