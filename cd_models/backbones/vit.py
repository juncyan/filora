...
class ViTB_patch16_512(nn.Layer):
    def __init__(self,
                 in_channels=3,
                 img_size=512,
                 patch_size=16,
        ...
        self.idx_skip = depth // 3
        self.out_sizes = [128,64,32]
        self.norms = [eval(norm_layer)(embed_dim, epsilon=epsilon)]* 3
    ...
    def reverse_embed(self, x, size):
        bs = x.shape[0]
        y = paddle.transpose(x, [0,2,1])
        y = paddle.reshape(y, [bs,-1])
        y = paddle.reshape(y, [bs, -1, size, size])
        return y
