...
class ImageEncoderViT(nn.Layer):
    ...
    def forward(self, x: paddle.tensor, y: paddle.tensor):
        ...
        y = self.neck(y.transpose([0, 3, 1, 2]))
...
class Block(nn.Layer):
    ...
    def __init__(
        ...
        self.mlp = MLPBlock(
            embedding_dim=dim, mlp_dim=int(dim * mlp_ratio), act=act_layer, rank=rank)
    ...
    def forward(self, x: paddle.tensor, y: paddle.tensor):
        ...
        if self.window_size > 0:
            ...
            y, _ = window_partition(y, self.window_size)
...
class Attention(nn.Layer):
    ...
    def __init__(
        ...
        self.qkv = lora.BiLinear(dim, dim * 3, bias_attr=qkv_bias if not qkv_bias else None, r=rank)
    ...
    def forward(self, x: paddle.tensor, y: paddle.tensor) -> paddle.tensor:
        ...
        qkv, qkvy = self.qkv(x, y)
        qkv = qkv.reshape(shape=[B, H * W, 3, self.num_heads, -1]).transpose([2, 0, 3, 1, 4])
        qkvy = qkvy.reshape(shape=[B, H * W, 3, self.num_heads, -1]).transpose([2, 0, 3, 1, 4])
        # q, k, v with shape (B * nHead, H * W, C)

        q, k, v = qkv.reshape(shape=[3, B * self.num_heads, H * W, -1]).unbind(
            axis=0)
        
        qy, ky, vy = qkvy.reshape(shape=[3, B * self.num_heads, H * W, -1]).unbind(
            axis=0)

        attn = (q * self.scale) @k.transpose([0, 2, 1])
        attny = (qy * self.scale) @ky.transpose([0, 2, 1])

        if self.use_rel_pos:
            attn = add_decomposed_rel_pos(attn, q, self.rel_pos_h,
                                          self.rel_pos_w, (H, W), (H, W))
            
            attny = add_decomposed_rel_pos(attny, qy, self.rel_pos_h,
                                          self.rel_pos_w, (H, W), (H, W))
        ...
        y = (attn @vy).reshape([B, self.num_heads, H, W, -1]).transpose(
            [0, 2, 3, 1, 4]).reshape([B, H, W, -1])
...
class MLPBlock(nn.Layer):
    def __init__(self,
        ...
        self.lin2 = lora.BiLinear(mlp_dim, embedding_dim, r = rank)#nn.Linear(mlp_dim, embedding_dim)
        self.act = act()
